...
export default function CadastroPublico() {
  ...
  useEffect(() => {
    ...
    (supabase as any).rpc("public_get_token_cadastro", { _token: t })
      .then(({ data, error }: any) => {
        if (error || !data) {
          setTokenInfo({ valido: false, motivo: error?.message ?? "Erro ao validar link" });
        } else {
          setTokenInfo(data);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const { data, error } = await (supabase as any).rpc("public_submit_token_cadastro", {
        _token: token ?? null,
        _nome: nome.trim(),
        _telefone: tel.replace(/\D/g, ""),
        _cpf: cpfDigits || null,
        _email: email || null,
        _lgpd: true,
      });

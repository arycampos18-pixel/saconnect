...
export function CompanySwitcher() {
  const { companies, currentCompany, changeCompany, isSuperAdmin } = useCompany();
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" role="combobox" aria-expanded={open} className="h-9 gap-2 max-w-[220px] justify-between">
            ...
            <CommandGroup>
              {companies.map((c) => (
                <CommandItem
                  ...
                  onSelect={async () => { await changeCompany(c.id); setOpen(false); }}
                  className="gap-2"
                >
